...
class RobertaForMultipleChoiceBag(BertPreTrainedModel):
    config_class = RobertaConfig
    pretrained_model_archive_map = ROBERTA_PRETRAINED_MODEL_ARCHIVE_MAP
    base_model_prefix = "roberta"

    def __init__(self, config):
        super(RobertaForMultipleChoiceBag, self).__init__(config)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None,
        ...
        pooled_output = self.dropout(pooled_output)
        x = pooled_output.view(bs * num_choices, num_bags, -1) # batch_size * num_choice, num_bags, hidden_size
        alphas = self.attention_ff(x)
        alphas = self.ff_before_alpha(alphas)
        alphas = torch.nn.functional.softmax(alphas.squeeze(-1), dim=-1)
