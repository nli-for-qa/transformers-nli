...
class PreTrainedTokenizer(object):
    ...
    @classmethod
    def _from_pretrained(cls, pretrained_model_name_or_path, *init_inputs,
                         **kwargs):
        ...
        if pretrained_model_name_or_path in s3_models:
            ...
            if (cls.pretrained_init_configuration
                    and pretrained_model_name_or_path in cls.
                    pretrained_init_configuration):
            ...
            if os.path.exists(
                    saved_directory) and not os.path.isdir(saved_directory):
    ...
    def add_tokens(self, new_tokens):
        ...
        self.unique_added_tokens_encoder = set(
            self.added_tokens_encoder.keys()).union(
            ...
            return list(
                itertools.chain.from_iterable(
                    (self._tokenize(token, **kwargs)
                     if token not in self.unique_added_tokens_encoder else
        ...
        if return_tensors is not None:
            ...
            for key, value in batch_outputs.items():
                ...
                if key != "input_len":
                    # Padding handle
                    padded_value = [
                        v + [self.pad_token_id if key == "input_ids" else 1]
                        * (max_seq_len - len(v)) for v in padded_value
                    ]
    ...
    def prepare_for_model(self,
        ...
        total_len = len_ids + len_pair_ids + (self.num_added_tokens(
        ...
        if max_length is None and len(
                encoded_inputs["input_ids"]) > self.max_len:
            logger.warning(
        ...
        if needs_to_be_padded:
            difference = (max_length if max_length is not None else
                          self.max_len) - len(encoded_inputs["input_ids"])
        ...
        out_string = (out_string.replace(" .", ".").replace(" ?", "?").replace(
            " !", "!").replace(" ,", ",").replace(" ' ", "'").replace(
                " n't", "n't").replace(" 'm", "'m").replace(
                    " do not", " don't").replace(" 's", "'s").replace(
                        " 've", "'ve").replace(" 're", "'re"))
...
class PreTrainedTokenizerFast(PreTrainedTokenizer):
    ...
    def decode_batch(self,
        ...
        return [
            self.clean_up_tokenization(text) if clear_up_tokenization_spaces
            else text for text in self.tokenizer.decode_batch(
