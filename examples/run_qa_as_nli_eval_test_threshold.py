...
def evaluate(args, model, tokenizer, istest, prefix=""):
    ...
    eval_dataset = load_and_cache_examples(
        args, eval_task_name, tokenizer, evaluate=not istest, test=istest)
    ...
    logger.info("***** Running evaluation on {} set {} *****".format(
        "test" if istest else "dev", prefix))
    ...
    if args.save_preds:
        pred_file = os.path.join(eval_output_dir, prefix,
                                 ("test" if istest else "eval") + "_preds.txt")
        with open(pred_file, "w") as f:
            writer = csv.writer(f)
            writer.writerow(preds)
        score_file = os.path.join(
            eval_output_dir, prefix,
            ("test" if istest else "eval") + "_scores.txt")
        with open(score_file, "w") as f:
            writer = csv.writer(f)
            writer.writerow(scores)
        label_file = os.path.join(
            eval_output_dir, prefix,
            ("test" if istest else "eval") + "_labels.txt")
        with open(label_file, "w") as f:
            writer = csv.writer(f)
            writer.writerow(out_label_ids)

    p = "test" if istest else "eval"
    result = {
        p + "_acc": acc,
    ...
    output_eval_file = os.path.join(
        eval_output_dir, prefix,
        ("test" if istest else "eval") + "_results.txt")

    with open(output_eval_file, "w") as writer:
        logger.info("***** " + ("Test" if istest else "Eval")
...
def main():
    ...
    args.use_threshold = True
    args.test = True
    logger.info(f"Using {args.threshold} as threshold for test")
    test_result = evaluate(args, model, tokenizer, istest=True)
    combined_result = {**eval_result, **test_result}

    if args.wandb:
        wandb_log(combined_result, step=0)
