            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [
                    ...
                    if (
                        ...
                        # because, for instance, for accuracy, acc= (TP+TN)/total,
                        # But if each gpu computes its own accuracy, then a1 = (TP1+TN1)/total1,
                        # and (a1+a2)/2 != (TP1+TP2+TN1+TN2)/(total1 + total2).

                        # We can eval on the main process using a single gpu and
                        # make other gpus wait till the val is finished
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        if args.n_gpu > 1:
            # this means model could already be an instance of torch.nn.DataParallel
            # because train method could have done that

            if isinstance(model, torch.nn.DataParallel):
                # we need to pull out the module object and
                # create another DataParallel instance with it
                # which only works on gpu 0
                module = model.module
            else:
                # the model is raw model
                module = model
            model = torch.nn.DataParallel(module, device_ids=[0])
